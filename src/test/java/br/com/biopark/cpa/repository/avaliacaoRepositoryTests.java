...
public class avaliacaoRepositoryTests {
    ...
    public void testePersistenciaAvaliacao() throws Exception {
        ...
        Eixo eixoPersistido = eixoRepository.findByNome(eixo.getNome(), pageable).getContent().get(0);

        Pergunta perguntaUm = new Pergunta("Pergunta 1", TipoPergunta.DESCRITIVA, true, eixoPersistido);
        entityManager.persist(perguntaUm);

        Pergunta perguntaDois = new Pergunta("Pergunta 2", TipoPergunta.AVALIATIVA, true, eixoPersistido);
        entityManager.persist(perguntaDois);

        Turma turmaUm = new Turma("Turma 1", "Noturno", curso, "turma teste");
        entityManager.persist(turmaUm);

        Turma turmaDois = new Turma("Turma 2", "Vespertino", curso, "Turma teste 2");
        ...
        Avaliacao avaliacao = new Avaliacao("Avaliação teste", listaPerguntas, listaTurmas, data);
        ...
        Assert.assertEquals(persistidoAvaliacao.getDataExpiracao(), avaliacao.getDataExpiracao());
